...
import win, stdrus, ll.c.Analysis, ll.c.Target, ll.c.Core, ll.c.Типы, ll.c.ExecutionEngine;
import ll.c.Transforms, ll.c.lto;
...
/+
    static if (useMCJIT) {
        ...
        +/
        ЛЛСоздайДжИТКомпиляторДляМодуля(&двиг, генМод, 2, cast(сим**) ошибка.вТкст0());
   // }

    if (ошибка)
    {
        масштаб (exit) ЛЛВыместиСообщение(ошибка.вТкст0());
...
цел main(ткст[] арги)
{
    ...
    win.скажинс(stdrus.вТкст(ll.c.lto.ЛЛОВК_ДайВерсию()));
