...
struct Type
{
    ...
    this (CXType cx)
    {
        ...
        конст_ли = clang_isConstQualifiedType(cx) == 1;
        isClang = да;
    }
    ...
    ткст вТкст() const
    {
        ...
        return format("Type(вид = %s, spelling = %s, конст_ли = %s)", вид, spelling, конст_ли);
    }
...
{
    this (ткст message, ткст file = __FILE__, т_мера строка = __LINE__)
    {
        super(message, file, строка);
    }
}

class TypeLayoutErrorUnknown : TypeLayoutError
{
    this (ткст spelling, ткст file = __FILE__, т_мера строка = __LINE__)
    {
...
class TypeLayoutErrorInvalidFieldName : TypeLayoutError
{
    this (ткст spelling, ткст file = __FILE__, т_мера строка = __LINE__)
    {
        super("The поле name '" ~ spelling ~ "' is not valid for this record.");
    }
}
...
проц throwTypeLayoutError(
    ...
    ткст spelling,
    ткст file = __FILE__,
    т_мера строка = __LINE__)
{
    final switch (layout)
    {
        case CXTypeLayoutError.invalid:
            throw new TypeLayoutErrorInvalid(spelling, file, строка);
        case CXTypeLayoutError.incomplete:
            throw new TypeLayoutErrorIncomplete(spelling, file, строка);
        case CXTypeLayoutError.dependent:
            throw new TypeLayoutErrorDependent(spelling, file, строка);
        case CXTypeLayoutError.notConstantSize:
            throw new TypeLayoutErrorNotConstantSize(spelling, file, строка);
        case CXTypeLayoutError.invalidFieldName:
            throw new TypeLayoutErrorInvalidFieldName(spelling, file, строка);
    }
}
