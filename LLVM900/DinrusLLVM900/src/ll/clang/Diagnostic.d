...
struct DiagnosticSet
{
    ...
    Diagnostic opIndex(т_мера индекс)
    {
        return Diagnostic(clang_getDiagnosticInSet(container.set, cast(uint) (begin + индекс)));
    }
...
бул hasError(DiagnosticSet diagnostics)
{
    ...
    return severity == CXDiagnosticSeverity.ошибка ||
