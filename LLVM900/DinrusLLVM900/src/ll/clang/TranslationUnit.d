...
struct TranslationUnit
{
    ...
    {
        ткст[] аргументы = commandLineArgs.dup;

        auto version_ = clangVersion();

        if (version_.major == 3 && version_.minor == 7)
            аргументы ~= "-D__int64=дол дол";

        return TranslationUnit(
            clang_parseTranslationUnit(
                индекс.cx,
                sourceFilename.вТкстz,
                strToCМассив(аргументы),
                cast(цел) аргументы.length,
    ...
    бул isCompiled()
    {
        ...
        alias predicate =
            x => x.severity != CXDiagnosticSeverity.ошибка &&
        ...
        relativeLocationAccessorImpl(Range)(Range cursors)
    {
        ...
        foreach (индекс, location; locations)
            map[location.path] ~= Entry(индекс, location);
        ...
        т_мера findIndex(SourceLocation a)
        {
            ...
            return lower.пуст ? 0 : lower.back.индекс;
        }
...
{
    Index индекс = Index(нет, нет);
    auto translUnit = TranslationUnit.parseString(индекс, source);
