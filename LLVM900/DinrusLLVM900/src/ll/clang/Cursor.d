...
struct Cursor
{
    ...
    private Cursor[] childrenImpl(Т)(бул ignorePredefined) 
    {
    ...
    Cursor opCast(Т)() /*const if (is(Т == Cursor)) */
	in {assert(is(Т == Cursor));}
    {
        return this;
    }

    бул opCast(Т)() /* if (is(Т == бул))*/
	in {assert(is(Т == бул));}
    {
        return !isEmpty && валидн_ли;
    }
...
struct FunctionCursor
{
    ...
    бул isVariadic ()
    {
        return тип.функц.isVariadic;
    }

    ParamVisitor параметры ()
    {
