...
//Ник для массива контекстов.
alias Контекст[] Контексты;
...
static this()
{
    ...
    public class Контекст
    {
		...
		{
				return ЛЛДайИДТипаМДВКонтексте(this.раскрой(), ll.common.вТкст0(имя), имя.length);
		}
...
/+
  ...
  +/
		...
		{
			ЛЛТип[] тЭлты;
			foreach(элт; типыЭлтов)
			{
				тЭлты ~= элт.раскрой();
			}
			return new ТипСтрукт(ЛЛТипСтруктВКонтексте(this.раскрой(), cast(ЛЛТип*)&тЭлты, тЭлты.length, упакован));
		}
		...
		{ 
			...
			return new Значение(ЛЛМДУзелВКонтексте(this.раскрой(), cast(ЛЛЗначение*)&значения, значения.length));
		}

        public БазБлок приставьБазБлок(Значение фн, ткст имя)
		{
			return new БазБлок(ЛЛПриставьБазБлокВКонтексте(this.раскрой(), фн.раскрой(), ll.common.вТкст0(имя)));
        }

        public БазБлок вставьБазБлокВКонтекст(БазБлок bb, ткст имя)
		{
			return new БазБлок(ЛЛВставьБазБлокВКонтекст(this.раскрой(), bb.раскрой(), ll.common.вТкст0(имя)));
		}
