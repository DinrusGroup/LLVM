...
namespace llc{
    ...
    extern "C" __declspec(dllexport) int ЛЛВхоФункцЛЛКомпилятора(char** args) {
        ...
        //////////////////////////////////////////
        cl::opt<std::string>
            ВхоФайлИмя(cl::Positional, cl::desc("<вводный биткод>"), cl::init("-"));

        cl::opt<std::string>
            ВхоЯзык("x", cl::desc("Язык ввода ('ir' или 'mir')"));

        cl::opt<std::string>
            ВыхФайлИмя("obj", cl::desc("Выходной файл"), cl::value_desc("имя_файла"));
        ...
        cl::opt<std::string>
            ТриадаЦели("mtriple", cl::desc("Переписать целевую триаду для модуля"));
        ...
        cl::opt<RunPassOption, true, cl::parser<std::string>> RunPass(
            ...
            cl::value_desc("имя_проходки"), cl::ZeroOrMore, cl::location(RunPassOpt));
        //////////////////////////////////////////////////////////////////////////////
        ...
        for (unsigned I = TimeCompilations; I; --I)
        {
            ...
            if (!SkipModule) {
                if (ВхоЯзык == "mir" ||
                    (ВхоЯзык == "" && StringRef(ВхоФайлИмя).endswith(".mir"))) {
                    MIR = createMIRParserFromFile(ВхоФайлИмя, Err, Context);
                    if (MIR)
                        M = MIR->parseIRModule();
                }
                else
                    M = parseIRFile(ВхоФайлИмя, Err, Context, false);
                if (!M) {
                ...
                if (!ТриадаЦели.empty())
                    M->setTargetTriple(Triple::normalize(ТриадаЦели));
                TheTriple = Triple(M->getTargetTriple());
            }
            else {
                TheTriple = Triple(Triple::normalize(ТриадаЦели));
            }
            ...
            if (ВыхФайлИмя.empty()) {
                if (ВхоФайлИмя == "-")
                    ...
                    // If ВхоФайлИмя ends in .bc or .ll, remove it.
                    StringRef IFN = ВхоФайлИмя;
                    if (IFN.endswith(".bc") || IFN.endswith(".ll"))
                        ВыхФайлИмя = IFN.drop_back(3);
                    else if (IFN.endswith(".mir"))
                        ВыхФайлИмя = IFN.drop_back(4);
                    else
                        ВыхФайлИмя = IFN;
            ...
            Out = llvm::make_unique<ToolOutputFile>(ВыхФайлИмя, EC, OpenFlags);
            if (EC) {
            ...
            if (!NoVerify && verifyModule(*M, &errs())) {
                std::string Prefix =
                    (Twine(args[0]) + Twine(": ") + Twine(ВхоФайлИмя)).str();
