...
static void VisitComponent(const std::string &Name,
  ...
  if (!AC) {
    errs() << "Не найден компонент: '" << Name << "' в карте. Доступны компоненты: ";
  ...
  assert(AC && "Неверное имя компонента!");
...
static std::vector<std::string> ComputeLibsForComponents(
  ...
  for (unsigned i = 0, e = Components.size(); i != e; ++i) {
    ...
    if (!ComponentMap.count(ComponentLower)) {
      llvm::errs() << "llvm-config: неизвестное имя компонента: " << Components[i]
                   << "\n";
      exit(1);
    }
...
static void usage() {
  errs() << "\
Использование: llvm-config <ОПЦИЯ>... [<КОМПОНЕНТ>...]\n\
\n\
Получить различную конфигурационную информацию, необходимую для конпиляции программ,\n\
использующих LLVM.  Обычно вызывается из скриптов 'configure'.  Примеры:\n\
  llvm-config --cxxflags\n\
  llvm-config --ldflags\n\
  llvm-config --libs engine bcreader scalaropts\n\
\n\
Опции:\n\
  --version         Вывести верию LLVM.\n\
  --prefix          Вывести префикс установки.\n\
  --src-root        Вывести исходный корень, из которого LLVM была построена.\n\
  --obj-root        Вывести объектный корень, используемый для построения LLVM.\n\
  --bindir          Папка, содержащая исполнимые модули LLVM.\n\
  --includedir      Папка, содержащая заголовочные файлы LLVM.\n\
  --libdir          Папка, содержащая библиотеки LLVM.\n\
  --cmakedir        Папка, содержащая LLVM cmake модули.\n\
  --cppflags        Флаги препроцессора C для файлов, включающих заголовки LLVM.\n\
  --cflags          Флаги компилятора C для файлов, включающих заголовки LLVM.\n\
  --cxxflags        Флаги компилятора C++ для файлов, включающих заголовки LLVM\n\
  --ldflags         Вывести флаги компоновщика.\n\
  --system-libs     Системные библиотеки, необходимые для линковки с компонентами LLVM.\n\
  --libs            библиотеки, необходимые для линковки к компонентам LLVM.\n\
  --libnames        Одни имена библитек для построения в дереве.\n\
  --libfiles        Полные имена библиотек для зависимостей makefile.\n\
  --components      Список всех возможных компонентов.\n\
  --targets-built   Список всех целей, на данный момент построенных.\n\
  --host-target     Целевая триада, мопользуемая для конфигурирования LLVM.\n\
  --build-mode      Вывести режим построения дерева LLVM (напр. Debug или Release).\n\
  --assertion-mode  Вывести режим ассертации дерева LLVM (ON или OFF).\n\
  --build-system    Вывести систему построения, использованную при постройке LLVM (всегда cmake).\n\
  --has-rtti        Вывести, построена ли LLVM с rtti (YES или NO).\n\
  --has-global-isel Вывести, построена ли LLVM с поддержкой global-isel (ON или OFF).\n\
  --shared-mode     Вывести, как предоставленные компоненты могут быть коллективно скомпонованы (`shared` или `static`).\n\
  --link-shared     Компоновать компоненты как совместные библиотеки.\n\
  --link-static     Компоновать компонентные библиотеки статически.\n\
  --ignore-libllvm  Игнорировать libLLVM и компоновать компонентные библиотеки.\n\
Типичные компоненты:\n\
  all               Все библиотеки LLVM (дефолт).\n\
  engine            Либо нативный JIT млм интерпретатор биткода.\n";
  exit(1);
}
...
extern "C" __declspec(dllexport) int ЛЛВхоФункцЛЛКонфиг(int argc, char **argv) {
  ...
  if (LinkMode == LinkModeShared && !DyLibExists && !BuiltSharedLibs) {
    WithColor::error(errs(), "llvm-config") << DyLibName << " отсутствует\n";
    return 1;
  }
  ...
  if (PrintLibs || PrintLibNames || PrintLibFiles || PrintSystemLibs ||
    ...
    if (!MissingLibs.empty()) {
      ...
      case LinkModeShared:
        ...
        for (auto &Lib : MissingLibs)
          WithColor::error(errs(), "llvm-config") << "отсутствует: " << Lib << "\n";
        return 1;
      case LinkModeAuto:
        if (DyLibExists) {
          LinkMode = LinkModeShared;
        ...
        WithColor::error(errs(), "llvm-config")
            << "компонентные библиотеки и совместная бибилиотека\n\n";
        LLVM_FALLTHROUGH;
      case LinkModeStatic:
        for (auto &Lib : MissingLibs)
          WithColor::error(errs(), "llvm-config") << "отсутствует: " << Lib << "\n";
    ...
    WithColor::error(errs(), "llvm-config")
        << "компонент задан, но не использован\n\n";
    usage();
  }
