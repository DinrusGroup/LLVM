...
static void WriteOutputFile(const Module *M, const ModuleSummaryIndex *Index) {
  ...
  if (ВыхИмяФ.empty()) {
    if (ВхоИмяФ == "-") {
      ...
      StringRef IFN = ВхоИмяФ;
      ВыхИмяФ = (IFN.endswith(".ll") ? IFN.drop_back(3) : IFN).str();
      ВыхИмяФ += ".bc";
    }
  }
...
extern "C" __declspec(dllexport) цел ЛЛВхоФункцЛЛАс(char **args) {
  ...
  cl::ParseCommandLineOptions(argn, args, "llvm .ll -> .bc ассемблер\n");

  // Parse the file now...
  SMDiagnostic Err;
  auto ModuleAndIndex = parseAssemblyFileWithIndex(
      ВхоИмяФ, Err, Context, nullptr, !DisableVerify, ClDataLayout);
